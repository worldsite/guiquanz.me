---
layout: post
title: 打印Lua提供的所有全局变量
category: lua
---
h2. Lua的全局变量

Lua解释器提供了很多全局变量，比如print等，便于程序开发。Lua提供的所有全局变量都保存在一个普通的表_G中。目前Lua-5.2.1中_G中的全局变量仅有“字符串”、“函数”及“表”三种。那该如何打印这些值呢？（当然，你在会话中进行的任何变更，都将对其造成影响）

h2. 如何打印Lua提供的所有全局变量？

既然_G是一个普通的表，那么我们可以采用for语句进行简单的遍历即可。具体代码，如下：
...
当然，为了打印所有的全局变量（比如，io库支持的所有函数等），我们需要对所有table类变量进行遍历（由于_G._G与_G等效，所以代码中进行了过滤，不重复打印。并且，此实现仅遍历了2层）。具体代码，如下：
